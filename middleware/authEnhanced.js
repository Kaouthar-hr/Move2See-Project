...
const authenticateToken = async (req, res, next) => {
    try {
        ...
        if (user.isDeleted) {
            console.log('Tentative d\'accès avec un compte supprimé', { userId: decoded.userId });
        ...
        if (!user.isActive) {
            console.log('Tentative d\'accès avec un compte inactif', { userId: decoded.userId });
            return res.status(403).json({
                success: false,
        ...
        console.log('Authentification réussie', { userId: user.id, role: user.role });
        next();
